...
class ZegoLiveStreamingManager implements ZegoLiveStreamingManagerInterface {
  ...
  ValueNotifier<ZegoLiveStreamingRole> currentUserRoleNotifier =
  ...
  ValueNotifier<ZegoSDKUser?> get hostNotifier =>
      cohostService?.hostNotifier ?? ValueNotifier(null);

  ListNotifier<ZegoSDKUser> get coHostUserListNotifier =>
      cohostService?.coHostUserListNotifier ?? ListNotifier([]);

  ValueNotifier<RoomPKState> get pkStateNotifier =>
      pkService?.pkStateNotifier ?? ValueNotifier(RoomPKState.isNoPK);

  ValueNotifier<bool> get isMuteAnotherAudioNotifier =>
      pkService?.isMuteAnotherAudioNotifier ?? ValueNotifier(false);

  ValueNotifier<bool> get onPKViewAvailableNotifier =>
      pkService?.onPKViewAvailableNotifier ?? ValueNotifier(false);
  ...
  Future<ZegoRoomLoginResult> startLive(String roomID) async {
    ...
    final result = await ZEGOSDKManager()
        .loginRoom(roomID, ZegoScenario.Broadcast, token: token);
    return result;
  }
  ...
  Future<ZegoMixerStartResult> mutePKUser(
    ...
    if (pkInfo != null) {
      ...
      return await pkService?.mutePKUser(muteIndex, mute) ??
          ZegoMixerStartResult(-9999, {});
    }
