...
class ZegoLiveAudioRoomSeat {
  final int seatIndex;
  final ValueNotifier<ZegoSDKUser?> currentUser;
  final ValueNotifier<bool> isLocked; // ✅ NEW FIELD

  ZegoLiveAudioRoomSeat(this.seatIndex)
      : currentUser = ValueNotifier(null),
        isLocked = ValueNotifier(false); // ✅ INITIALLY UNLOCKED
}
...
class RoomSeatService {
  ...
  void updateCurrentUserRole() {
    ...
    for (final seat in seatList) {
      if (seat.currentUser.value?.userID == ZEGOSDKManager().currentUser!.userID) {
        isFindSelf = true;
        break;
      }
    }
