...
class _MessageScreenState extends State<MessageScreen> {
  ...
  void openStartCallSheet(bool isVideo) {
    // Check permissions before showing call sheet
    _checkCallPermissions(isVideo);
  }

  Future<void> _checkCallPermissions(bool isVideo) async {
    try {
      print(
          '🔐 Checking call permissions for ${isVideo ? 'video' : 'audio'} call...');

      // Check microphone permission (required for both audio and video calls)
      ...
      bool hasMicPermission = micStatus.isGranted;
      bool hasCameraPermission = cameraStatus.isGranted;

      if (isVideo && (!hasMicPermission || !hasCameraPermission)) {
        print('⚠️ Missing permissions for video call - requesting...');
        await _requestCallPermissions(isVideo);
      } else if (!isVideo && !hasMicPermission) {
        print(
            '⚠️ Missing microphone permission for audio call - requesting...');
        await _requestCallPermissions(isVideo);
      } else {
        print('✅ All permissions granted - showing call sheet');
        _showCallSheet(isVideo);
      }
    } catch (e) {
      print('❌ Error checking call permissions: $e');
      // Show call sheet anyway and let Zego handle the error
      _showCallSheet(isVideo);
    }
  }

  Future<void> _requestCallPermissions(bool isVideo) async {
    try {
      ...
      bool cameraGranted =
          isVideo ? (statuses[Permission.camera]?.isGranted ?? false) : true;
